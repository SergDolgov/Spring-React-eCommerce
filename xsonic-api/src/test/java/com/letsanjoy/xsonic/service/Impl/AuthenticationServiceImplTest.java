...
import com.letsanjoy.xsonic.security.oauth2.*;
...
public class AuthenticationServiceImplTest {
    ...
    public void login() {
        ...
        verify(tokenProvider, times(1)).generate(user.getEmail(), user.getRoles().iterator().next().name());
    }
    ...
    public void registerFacebookOauthUser() {
        ...
        user.setActive(true);
        user.setRoles(Collections.singleton(Role.USER));
        user.setProvider(OAuth2Provider.FACEBOOK);
    ...
    public void updateOauthUser() {
        ...
        assertEquals(FIRST_NAME, user.getFirstName());
        assertEquals(LAST_NAME, user.getLastName());
        assertEquals(OAuth2Provider.GOOGLE, user.getProvider());
        assertNull(user.getPassword());
        verify(userRepository, times(1)).save(user);
    }
